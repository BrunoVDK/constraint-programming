...
% @param N      The dimension of the puzzle.
% @param K      The dimension of blocks.
generate_constraints_combined(Primal, _Member, N, K) :-
    % Primal
    ...
    (multifor([I,V], 1, N), param(N, Primal) do
        ic_global:occurrences(V, Primal[I,1..N], 1),
        ic_global:occurrences(V, Primal[1..N,I], 1)
    ),
    (multifor([V,R,C], 1, N, [1,K,K]), param(K, Primal) do
        ic_global:occurrences(V, flatten(Primal[R..R+K-1,C..C+K-1]), 1)
    ),
    % Channeling
